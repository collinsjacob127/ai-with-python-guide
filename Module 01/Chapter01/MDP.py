...
PossibleAction = possible_actions(R, agent_s_state)

# This function chooses at random which action to be performed within the range
# of all the available actions.
def ActionChoice(available_actions_range):
    if sum(available_actions_range) > 0:
        next_action = int(ql.random.choice(available_actions_range, 1))
    else:  # sum possibleaction <= 0
...
def reward(Q, current_state, action, gamma):
    # Find best state from our current action.
    Max_State = ql.where(
        Q[
            action,
        ]
...
# Rewarding Q matrix
reward(Q, agent_s_state, action, gamma)
...
for i in range(50000):
    ...
    PossibleAction = possible_actions(R, current_state)
    action = ActionChoice(PossibleAction)
    reward(Q, current_state, action, gamma)
